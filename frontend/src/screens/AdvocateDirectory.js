    ...
    <div className="min-h-screen bg-[#F8FAFC]">
      ...
      <div className="ns-page">
        <div className="max-w-7xl mx-auto">
          <h1 className="ns-heading-xl mb-8" data-testid="directory-title">
          ...
          <div className="bg-white border border-slate-200 shadow-sm rounded-sm p-6 mb-8" data-testid="filter-section">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div>
                ...
                <Input data-testid="location-input" placeholder="Enter city or state" value={location} onChange={(e) => setLocation(e.target.value)} className="h-12 bg-white border-slate-200 rounded-sm" />
              </div>
              <div className="flex items-end">
                <Button onClick={handleFilter} data-testid="filter-btn" className="w-full bg-[#B45309] text-white hover:bg-[#B45309]/90 h-12 rounded-sm font-medium">
                  Apply Filters
                </Button>
              </div>
            </div>
                    ...
                    <div className="bg-white border border-slate-100 shadow-sm hover:shadow-md transition-all p-0 overflow-hidden group rounded-sm" data-testid={`advocate-card-${advocate.id}`}>
                      <div className="p-6">
                        <div className="flex items-start justify-between mb-4">
                          <div>
                            <h3 className="text-xl font-semibold serif text-[#0F172A] mb-1">{advocate.name}</h3>
                            <p className="text-sm text-slate-600 mono">{advocate.bar_council_number}</p>
                          </div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {advocate.specializations.slice(0, 3).map((spec, idx) => (
                            <span key={idx} className="bg-slate-100 text-slate-700 px-3 py-1 text-xs font-medium rounded-sm">{spec}</span>
                          ))}
                        </div>
                      </div>
                      <div className="bg-slate-50 px-6 py-3 border-t border-slate-100">
                        <span className="text-sm font-medium text-[#B45309] group-hover:underline">View Profile {'>'}</span>
                      </div>
                    </div>
                  </Link>
                )
